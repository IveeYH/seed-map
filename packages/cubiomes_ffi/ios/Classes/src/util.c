...
Pos wrapper_estimateSpawn(const Generator *g, uint64_t *rng) {
    return estimateSpawn(g, rng);
}

__attribute__((used)) __attribute__((visibility("default")))
int wrapper_getStructurePos(int structureType, int mc, uint64_t seed, int regX, int regZ, Pos *pos) {
    return getStructurePos(structureType, mc, seed, regX, regZ, pos);
}

__attribute__((used)) __attribute__((visibility("default")))
int wrapper_isViableStructurePos(int structType, const Generator *g, int blockX, int blockZ, uint32_t flags) {
    return isViableStructurePos(structType, g, blockX, blockZ, flags);
}

__attribute__((used)) __attribute__((visibility("default")))
void wrapper_setupGenerator(Generator *g, int mc, uint32_t flags) {
    setupGenerator(g, mc, flags);
}

__attribute__((used)) __attribute__((visibility("default")))
void wrapper_applySeed(Generator *g, int dim, uint64_t seed) {
    applySeed(g, dim, seed);
}

__attribute__((used)) __attribute__((visibility("default")))
int wrapper_getBiomeAt(const Generator *g, int scale, int x, int y, int z) {
